...
export default function ELectionIndex({ election, candidates, result }: { election: object; candidates: Array<object>; result: Array }) {
    return (
                            ...
                            <ul className="list-disc pl-5">
                                {candidates.map((candidate: object) => (
                                    <li key={candidate.id} className="mt-2">
                                        {candidate.name}
                                        <span className="ml-2 inline-block rounded bg-green-600 px-3 py-0.5 text-xs text-white lg:text-sm">
                                            {result[candidate.id] ?? 0} votes
                                        </span>
                                    </li>
                                ))}
                            </ul>
